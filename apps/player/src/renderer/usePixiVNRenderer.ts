...
export function usePixiVNRenderer(onChoose: (optionId: string) => void, onActionSequenceComplete?: () => void) {
  /** 当前渲染器实例。 */
  const renderer = shallowRef<PixiVNRenderer | null>(null);

  /** 挂载渲染器。 */
  async function mount(container: HTMLElement): Promise<void> {
    renderer.value = new PixiVNRenderer({ onChoose, onActionSequenceComplete });
