...
桌面端继续复用 `apps/editor` 的编辑界面和工作流能力，包括多脚本管理、节点排序、复制/剪切/粘贴、撤销/重做、搜索筛选、校验定位和快捷键。桌面壳只提供本地工程读写、素材复制和 Web 游戏包导出能力，不成为业务逻辑中心。

## Prop 素材导入

桌面端素材导入支持 `prop`，文件会复制到当前工程的 `assets/prop/`，项目 JSON 只登记工程内相对路径，不写入本地绝对路径。
