...
- 顶部菜单栏：项目、编辑、视图、预览、构建、导出、帮助，以及新建、打开、保存、撤销、重做、预览和构建等入口。
...
当编辑器运行在 Tauri 中时，菜单和状态栏会显示桌面模式，并启用新建工程、打开工程、保存工程、导入本地素材、导出完整 Web 游戏包等能力。Web 模式下桌面专属菜单项禁用，仍保留 JSON 导入导出和素材元数据手动编辑能力。
