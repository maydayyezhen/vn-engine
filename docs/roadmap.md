...
## 第19轮：编辑器工作台 UI/UX 整体重构

状态：已完成第一版工作台信息架构重构。

- `apps/editor` 已改为专业创作工具式布局：顶部菜单栏/工具栏、左侧资源管理器、中央大画面预览、中央下方脚本与资源工作区、右侧检查器、底部状态栏。
- 场景预览被提升为中央主工作区，保留 16:9 画面预览和预览控制。
- 脚本编辑改为指令表形态，保留新增、复制、剪切、粘贴、删除、上移、下移、撤销、重做、搜索和筛选入口。
- 素材、角色、变量、动画、导出和校验入口被重新组织到工作区、资源管理器和右侧检查器中。
- 本轮只调整编辑器 UI/UX 和入口组织，不修改核心 schema/runtime/renderer/audio/project/export 语义，不新增节点图、关键帧时间轴或 AI 能力。
...
当前新增 `examples/showcase-game`，用于以完整短篇工程验证播放器、编辑器、动画模块、音频、变量分支、存档读档和 Web 导出包。Showcase 已接入 Potat0Master 背景、Xiael 透明角色部件组合、OpenGameArt BGM、Kenney 音效和 Wikimedia Commons 物品图片；语音仍为本地技术占位，后续可继续替换为真实配音。

本轮没有新增引擎大功能，没有重做 UI，也没有接入 AI API。后续更适合继续补 PropLayer、正式素材导入体验和 UI/UX 总体重构。

## 第17轮：PropLayer 物品展示

已加入正式 prop 资源类型、ShowPropNode/HidePropNode、运行时 props 状态、PixiJS PropLayer、prop 动画模块、编辑器物品节点入口、桌面 assets/prop 导入目录和 Showcase 旧信物品展示。
