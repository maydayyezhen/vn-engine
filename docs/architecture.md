...
`schema` 是数据契约，`core` 是解释器，`renderer` 是画面适配，`editor` 是数据编辑界面。分离后，运行时可以被播放器、编辑器预览和测试工具共同复用，不会被 Vue 页面或具体渲染实现绑定。
...
播放器只负责读取项目数据、驱动 `VNRuntime`、展示 `RuntimeSnapshot`。它不保存工程，不编辑节点，也不直接解析底层文件结构。
...
当前编辑器通过浏览器能力导入和下载 JSON 文件，序列化规则集中在 `packages/vn-project`。这样可以先验证项目数据保存与恢复闭环，避免过早把工程格式绑定到 Tauri 文件系统。未来桌面壳会复用同一套纯逻辑。

## 为什么素材库先做元数据

本阶段只管理素材 id、名称、类型、路径和说明，不处理真实二进制文件。这样可以先打通节点引用、校验、导入导出和预览展示，后续再接真实 Tauri 文件系统和素材导入。
...
Element Plus 适合编辑器中的表单、树、列表、弹窗和面板，但不应该污染核心包或播放器。核心包必须 UI 解耦，播放器也不应该呈现后台管理风格。
