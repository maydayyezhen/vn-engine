...
`PlayAnimationNode` 用于调用代码型动画模块。动画代码本身不写入项目 JSON，项目只保存动画 id、目标和参数。

```json
{
  "id": "play-animation-soft-enter",
  "type": "playAnimation",
...
- `animationId`：引用 `vn-renderer-pixi` 中注册的动画模块 id。
- `targets`：按动画模块的 `targetSlots` 填写目标。角色目标保存角色 id；镜头、屏幕、粒子目标通常不需要 id。
- `params`：按动画模块的 `paramsSchema` 填写参数。
- `waitForCompletion`：为 `true` 时，播放器等待动画完成后再继续剧情。
- `autoNext`：为 `true` 时，动画完成后由播放器协调运行时进入下一节点。

旧的 `ActionSequenceNode` 仍保留兼容；新的可复用定制动画优先使用 `PlayAnimationNode`。

## Prop 物品节点

`AssetItem.type` 现在支持 `prop`。`ShowPropNode` 用于显示或更新物品：`propId` 是画面对象 id，`assetId` 指向 prop 素材，支持 `x/y/scale/opacity/zIndex/rotation/flipX`，可选 `enterAnimationId` 和 `enterParams`。`HidePropNode` 用于隐藏物品，可选 `exitAnimationId` 和 `exitParams`。物品动画是一次性事件，静态状态保存在 `snapshot.props`。
